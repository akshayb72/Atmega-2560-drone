...
//Safety note
///////////////////////////////////////////////////////////////////////////////////////
//Always remove the propellers and stay away from the motors before testing anything
//Other than when you are flying the drone in an open area
///////////////////////////////////////////////////////////////////////////////////////
...
void setup(){
  ...
  DDRC |= B00001111;                                                        //Configure digital port 37, 36, 35and 34as output.
  DDRB |= B11000000;                                                        //Configure digital port 12 and 13 as output.
  ...
  for (cal_int = 0; cal_int < 1250 ; cal_int ++){                           //Wait 5 seconds before continuing.
    PORTC |= B00001111;                                                     //Set digital port 34,35,36 and 37 high.
    delayMicroseconds(1000);                                                //Wait 1000us.
    PORTC &= B11110000;                                                   //Set digital port34,35,36    and 37 low.
  ...
  for (cal_int = 0; cal_int < 2000 ; cal_int ++){                           //Take 2000 readings for calibration.
    ...
    PORTC |= B00001111 ;                                                   //Set digital port 34,35,36 and 37 high.
    delayMicroseconds(1000);                                                //Wait 1000us.
    PORTC &= B11110000;                                                     //Set digital port 34,35,36 and 37 low.
  ...
  while(receiver_input_channel_3 < 990 || receiver_input_channel_3 > 1020 || receiver_input_channel_4 < 1400){
    ...
    //We don't want the esc's to be beeping annoyingly. So let's give them a 1000us puls while waiting for the receiver inputs.
                  PORTC &= B00001111;                                //Set digital port 34,35,36 and 37 high.
    delayMicroseconds(1000);                                                //Wait 1000us.
     PORTC |= B11110000;                                                  //Set digital port 34,35,36 and 7 low.
    delay(3);                                                               //Wait 3 milliseconds before the next loop.
...
void loop(){
  ...
  PORTC |= B00001111;                                                   //Set digital outputs 34,35, 36 and 37 high.
  ...
  while(PORTC >= 16){                                                       //Stay in this loop until output 34,35,36 and 37are low.
    esc_loop_timer = micros();                                              //Read the current time.
    if(timer_channel_1 <= esc_loop_timer)PORTC &= B11111110;                //Set digital output 37to low if the time is expired.
    if(timer_channel_2 <= esc_loop_timer)PORTC &= B11111101;                //Set digital output 36 to low if the time is expired.
    if(timer_channel_3 <= esc_loop_timer)PORTC &= B11111011;                //Set digital output 35to low if the time is expired.
    if(timer_channel_4 <= esc_loop_timer)PORTC &= B11110111;                //Set digital output 34 to low if the time is expired.
  }
}
