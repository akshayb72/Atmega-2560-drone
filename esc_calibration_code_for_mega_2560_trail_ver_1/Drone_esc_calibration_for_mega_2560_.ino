...
void setup(){
  ...
  DDRC |= B00001111;                                                                    //Configure digital poort 4, 5, 6 and 7 as output.
  DDRB |= B01000000;                                                                    //Configure digital poort 12 as output.
  ...
  if(data == 'a'){
    ...
    if(cal_int != 2000){
      ...
      for (cal_int = 0; cal_int < 2000 ; cal_int ++){                                   //Take 2000 readings for calibration.
        ...
        PORTC |= B00001111;                                                             //Set digital poort 4, 5, 6 and 7 high.
        delayMicroseconds(1000);                                                        //Wait 1000us.
        PORTC &= B11110000;                                                             //Set digital poort 4, 5, 6 and 7 low.
...
void esc_pulse_output(){
  zero_timer = micros();
  PORTC |= B00001111;                                            //Set port 4, 5, 6 and 7 high at once
  ...
  while(PORTC >= 16){                                            //Execute the loop until digital port 4 to 7 is low.
    ...
    if(timer_channel_1 <= esc_loop_timer)PORTC &= B11111110;     //When the delay time is expired, digital port 37 is set low.
    if(timer_channel_2 <= esc_loop_timer)PORTC &= B11111101;     //When the delay time is expired, digital port 36 is set low.
    if(timer_channel_3 <= esc_loop_timer)PORTC &= B11111011;     //When the delay time is expired, digital port 35 is set low.
    if(timer_channel_4 <= esc_loop_timer)PORTC &= B11110111;     //When the delay time is expired, digital port 34 is set low.
  }
}
